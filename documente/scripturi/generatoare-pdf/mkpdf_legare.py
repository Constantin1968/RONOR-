...
S = {
 ...
 "code":  st("code", fontName="Mono", fontSize=7.7, leading=10.6,
             textColor=HexColor("#28251D"), spaceAfter=0,
             leftIndent=7, rightIndent=4),
}
...
def build(md):
    ...
    while i < len(lines):
        ...
        # bloc de cod delimitat cu ``` (orice limbaj)
        if s.startswith("```"):
            i += 1
            code = []
            while i < len(lines) and not lines[i].strip().startswith("```"):
                ...
                i += 1
            i += 1  # sare peste delimitatorul de inchidere
            ...
            if code:
                ...
                for c in code:
                    txt = esc(c).replace(" ", "&nbsp;")
                    inner.append(Paragraph(txt or "&nbsp;", S["code"]))
                box = Table([[inner]], colWidths=[CW], hAlign="LEFT")
                box.setStyle(TableStyle([
                    ("BACKGROUND", (0, 0), (-1, -1), SURF2),
                    ("BOX", (0, 0), (-1, -1), 0.5, RULE),
                    ("LINEBEFORE", (0, 0), (0, -1), 2.0, TEAL),
...
def cover():
    o = [Spacer(1, 28 * mm)]
    o.append(Paragraph("LEGAREA PERSISTENȚEI", S["cover_t"]))
    o.append(Paragraph("SUVERANE", S["cover_t"]))
    o.append(Spacer(1, 5))
    ...
    o.append(Spacer(1, 3))
    o.append(Paragraph("Cauza verdelui fals, reparația probată și poarta de integrare umană", S["cover_s2"]))
    o.append(Spacer(1, 15 * mm))
    rows = [
        ["Obiect", "Legarea lanțului de audit la baza de guvernanță suverană și eliminarea sănătății false"],
        ["Data", "30 august 2026"],
        ["Straturi de cauză", "3 — adaptor către bază externă, rețele disjuncte, strat de persistență nechemat"],
        ["Metodă", "Probă de scriere cap-coadă, cerere reală pe runtime, numărarea apelurilor în sursă și în compilat"],
        ["Rezultat de cod", "Cerere de integrare #30 — 9 fișiere, 6 verificări obligatorii trecute, ramura principală neatinsă"],
        ["Regim", "Niciun merge, push, release sau deploy fără acord explicit"],
        ["Clasificare", "Intern — conține referințe la defecte neremediate"],
    ]
    ...
    o.append(Paragraph(
        "Toate constatările din acest raport au fost măsurate direct, prin probe de scriere executate "
        "în infrastructura de producție, cereri reale pe serviciul viu și citirea codului care rulează. "
        "Nicio valoare de secret nu este afișată. Nicio operațiune de fuziune, publicare, "
        "lansare sau desfășurare nu a fost efectuată.", S["note"]))
    o.append(PageBreak())
    return o
...
def deco(cv, doc):
    ...
    if doc.page > 1:
        ...
        cv.drawString(LM, PH - TM + 7.5 * mm,
                      "RONOR — Legarea persisten\u021bei suverane \u00b7 30 august 2026")
        ...
        cv.setFillColor(FAINT)
        cv.drawRightString(PW - RM, BM - 9 * mm, "30 august 2026")
    cv.restoreState()

def main(src, out):
    md = open(src, encoding="utf-8").read()
    # taie blocul de titlu deja randat pe copert\u0103
    m = re.search(r"^## 1\. Sinteza", md, re.M)
    body = md[m.start():] if m else md
    doc = BaseDocTemplate(out, pagesize=A4,
                          leftMargin=LM, rightMargin=RM,
                          topMargin=TM, bottomMargin=BM,
                          title="RONOR — Legarea persistenței suverane, 30 august 2026",
                          author="NrgPaths Advisory Ltd",
                          subject="Legarea persistenței suverane RONOR",
