...
if docker exec ronor-dify-db sh -c 'pg_dumpall -U ${POSTGRES_USER:-postgres}' > "$D/volume/dify.sql" 2>>"$LOG"; then
  ...
  if [ "${LN:-0}" -ge 20 ]; then
    ...
    log "  [ESEC] dify.sql are doar $LN linii"
  fi
else
  log "  [ESEC] pg_dumpall ronor-dify-db"
fi
...
for v in cida_cida_lake n8n_n8n_data prefect_prefect_data; do
  ...
  if [ "$RCV" -le 1 ] && [ "${NFV:-0}" -ge 1 ]; then
    ...
    log "  [ESEC] $v cod=$RCV fisiere=$NFV"
  fi
done
...
if [ "${SECN:-0}" -lt 20 ]; then
  log "  [ESEC] doar ${SECN:-0} fisiere de mediu gasite (minim asteptat 20);"
  ...
  if [ "${AN:-0}" -lt "${SECN:-0}" ]; then
    log "  [ATENTIE] arhiva are $AN din $SECN fisiere asteptate"
  fi
  STATUS_SECRETE=ok
fi
rm -f "$SECL"
