#!/usr/bin/env bash
# Installs only the corrected event-log artifact reference, on top of the
# installed context stage, into the isolated development stack. Replaces
# exactly two service images: the controller and the OpenHands bridge. Does
...
# baseline.
set -Eeuo pipefail
[[ "${1:-}" == --approved-artifact ]] || { echo 'Explicit approval required' >&2; exit 2; }
...
[[ -f "$root/context.env" ]] || { echo context_stage_required >&2; exit 2; }
[[ ! -e "$root/artifact.env" ]] || { echo artifact_overlay_already_present >&2; exit 2; }
phase=preflight
trap 'echo "artifact_install_failed_phase=$phase; stopped_without_retry_or_rollback" >&2' ERR
umask 077
...
for name in environment verification-fix.env controller-fix.env accounting-fix.env \
            budget.env recovery-fix.env wirefix.env transport.env egress.env context.env; do
  [[ -f "$root/$name" ]]
...
export RONOR_ARTIFACT_SOURCE="$source_dir" RONOR_ARTIFACT_TAG="$revision"
compose+=(-f "$source_dir/docker-compose.development-artifact.yml")
...
[[ "$(docker inspect ronor-development-openhands-bridge-1 --format '{{.Config.Image}}')" == "ronor-artifact-runtime:$revision" ]]
[[ "$(docker exec ronor-development-controller printenv RONOR_AUTOMATION_MAX_COST_USD)" == 100 ]]
phase=record
printf 'RONOR_ARTIFACT_SOURCE=%s\nRONOR_ARTIFACT_TAG=%s\n' "$source_dir" "$revision" > "$root/artifact.env"
printf '%s\n' "$before" > "$source_dir/artifact-preserved-state.json"
printf '%s\n' "$before_containers" > "$source_dir/artifact-containers-before.txt"
containers > "$source_dir/artifact-containers-after.txt"
printf 'ledger_sha256=%s\n' "$before_ledger" > "$source_dir/artifact-ledger-unchanged.txt"
echo 'artifact_fix_installed; controller_and_bridge_only; six_other_containers_unchanged; ledger_and_run_state_unchanged; no_model_started'
