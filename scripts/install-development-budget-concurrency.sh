#!/usr/bin/env bash
# Roll the already-installed wirefix overlay forward to a reviewed revision that
# admits bounded concurrent dispatch in the model budget ledger. Exactly three
# services are replaced. Observational guarantees: no reauthorization, no model
# call, no run started, no ledger write. Refuses instead of improvising; never
# retries and never rolls forward on failure.
set -Eeuo pipefail
[[ "${1:-}" == --approved ]] || exit 2
...
for file in environment verification-fix.env controller-fix.env accounting-fix.env budget.env recovery-fix.env wirefix.env; do
  [[ -f "$root/$file" ]] || { echo "missing_overlay=$file" >&2; exit 2; }
done
# This is a forward roll of an installed overlay, not a first install.
installed_tag="$(sed -n 's/^RONOR_WIREFIX_TAG=//p' "$root/wirefix.env")"
[[ "$installed_tag" =~ ^[a-f0-9]{40}$ ]] || exit 2
[[ "$installed_tag" != "$revision" ]] || { echo revision_already_installed >&2; exit 2; }
...
phase=preflight
trap 'echo "budget_concurrency_install_failed_phase=$phase; stopped_without_retry_or_rollback" >&2' ERR
umask 077
...
printf 'previous_tag=%s\nnew_tag=%s\n' "$installed_tag" "$revision" > "$source_dir/budget-concurrency-forward-roll.txt"
printf '%s\n' "$before_state" > "$source_dir/budget-concurrency-preserved-state.json"
printf '%s\n' "$before" > "$source_dir/budget-concurrency-containers-before.txt"
printf '%s\n' "$after" > "$source_dir/budget-concurrency-containers-after.txt"
printf 'ledger_sha256=%s\n' "$before_ledger" > "$source_dir/budget-concurrency-ledger-unchanged.txt"
echo 'budget_concurrency_installed; three_services_replaced; ledger_and_run_state_unchanged; no_model_started'
