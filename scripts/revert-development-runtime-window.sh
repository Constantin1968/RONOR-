#!/usr/bin/env bash
# Withdraws the one-probe runtime window and returns the controller to its
# standing explicit bound of 15 minutes. Recreates exactly one container, the
...
set -Eeuo pipefail
[[ "${1:-}" == --approved-runtime-window-revert ]] || { echo 'Explicit approval required' >&2; exit 2; }
[[ "$(id -u)" == 0 ]] || exit 2
root=/srv/ronor/development-automation
[[ -f "$root/artifact.env" && -f "$root/evidence-home.env" ]] || { echo prior_stages_required >&2; exit 2; }
[[ -f "$root/runtime-window.env" ]] || { echo runtime_window_overlay_absent >&2; exit 2; }
phase=preflight
trap 'echo "runtime_window_revert_failed_phase=$phase; stopped_without_retry_or_rollback" >&2' ERR
umask 077
...
[[ "$(docker exec ronor-development-controller printenv RONOR_AUTOMATION_MAX_RUNTIME_MINUTES)" == 45 ]]
...
[[ -z "$(docker exec -e GIT_OPTIONAL_LOCKS=0 ronor-development-controller git -C /automation-worktrees/project status --porcelain)" ]]
phase=record
printf '%s\n' "$before_containers" > "$root/runtime-window-revert-containers-before.txt"
containers > "$root/runtime-window-revert-containers-after.txt"
printf 'ledger_sha256=%s\ncontroller_image=%s\nprevious_bound_minutes=45\nrestored_bound_minutes=15\n' \
  "$before_ledger" "$before_image" > "$root/runtime-window-revert-record.txt"
echo "runtime_window_withdrawn; controller_only; same_image; bound_back_to_15; seven_other_containers_unchanged; cost_ceiling_unchanged; ledger_and_run_state_unchanged; no_model_started"
