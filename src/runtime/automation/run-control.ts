...
export function registerAutomationRun(runId: string, missionId: string): { signal: AbortSignal; finish(): void } | null {
  if (active.has(runId)) return null;
  const controller = new AbortController();
  active.set(runId, { missionId, controller });
  return { signal: controller.signal, finish: () => { active.delete(runId); } };
}
